...
-- Modified for R15-to-R6 conversion
...
-- List of body parts to synchronize (R15 parts)
local bodyParts = {
...
-- R6 to R15 part mapping for conversion
...
-- R15 to R6 part mapping for conversion
local R15toR6Mapping = {
    ["Head"] = "Head",
    ["UpperTorso"] = "Torso",
    ["LowerTorso"] = "Torso",
...
-- Built-in R6 Animations (Example - you can expand this table)
local BuiltInAnimationsR6 = {
    ...
    ["Tantrum"] = "4966882981",
    ["Headless"] = "35154961",
    ["Helicopter"] = "752634581",
    ["Breakdance"] = "429730430",
    ["Shuffle"] = "429730143",
    ["Twirl"] = "776635125",
    ["Samba"] = "6869810980",
    ["Curtsy"] = "4940561610",
    ...
    ["Spasm"] = "3823158750",
    ["Monkey"] = "3716636630",
    ["Gangnam Style"] = "11710529975",
    ["Macarena"] = "11710527872"
}
    ...
    if success then
        ...
        if decodeSuccess and typeof(decodedCustom) == "table" then
            ...
            for animName, animId in pairs(customAnimations) do
                if not BuiltInAnimationsR6[animName] then
                    BuiltInAnimationsR6[animName] = animId
...
local function loadKeybinds()
    ...
    if success then
        local decodeSuccess, decodedKeybinds = pcall(function()
            return HttpService:JSONDecode(fileContent)
        end)
        ...
        warn("No keybinds file found, starting with empty keybinds")
        animationKeybinds = {}
    end
end
...
-- Function to convert R15 character to R6-like structure for animations
local function convertToR6Structure(character)
    if not character then return end
    
    -- Check if character is already R6
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if humanoid and humanoid.RigType == Enum.HumanoidRigType.R6 then
        print("Character is already R6, no conversion needed")
        return character
    end
    
    print("Converting R15 character to R6-like structure...")
    
    -- Create R6 parts if they don't exist
    local r6Parts = {"Head", "Torso", "Left Arm", "Right Arm", "Left Leg", "Right Leg"}
    local existingParts = {}
    
    -- First, check which R6 parts already exist
    for _, partName in ipairs(r6Parts) do
        local part = character:FindFirstChild(partName)
        if part then
            existingParts[partName] = part
        end
    end
    
    -- Create missing R6 parts
    for _, partName in ipairs(r6Parts) do
        if not existingParts[partName] then
            local part = Instance.new("Part")
            part.Name = partName
            part.Size = Vector3.new(1, 1, 1) -- Default size, will be adjusted
            part.Transparency = 1 -- Make invisible
            part.CanCollide = false
            part.Anchored = false
            part.Parent = character
            
            -- Create welds to connect R6 parts to corresponding R15 parts
            if partName ~= "Head" then -- Head already exists in both rigs
                local weld = Instance.new("Weld")
                weld.Name = partName .. "Weld"
                weld.Part0 = part
                
                -- Find the primary R15 part to weld to
                local r15Parts = R6toR15Mapping[partName]
                if type(r15Parts) == "table" then
                    local primaryPart = character:FindFirstChild(r15Parts[1])
                    if primaryPart then
    ...
    -- Adjust sizes and positions of R6 parts based on R15 parts
    -- Torso
    local upperTorso = character:FindFirstChild("UpperTorso")
    local lowerTorso = character:FindFirstChild("LowerTorso")
    local torso = character:FindFirstChild("Torso")
    
    if upperTorso and lowerTorso and torso then
        -- Set torso size to encompass both upper and lower torso
        local upperSize = upperTorso.Size
        local lowerSize = lowerTorso.Size
        ...
        -- Position torso at the center between upper and lower torso
        local upperPos = upperTorso.Position
        local lowerPos = lowerTorso.Position
        torso.Position = (upperPos + lowerPos) / 2
    end
    ...
    if leftArm and leftUpperArm and leftLowerArm and leftHand then
        -- Set left arm size to encompass all left arm parts
        local upperSize = leftUpperArm.Size
        local lowerSize = leftLowerArm.Size
        local handSize = leftHand.Size
        leftArm.Size = Vector3.new(
            upperSize.X,
            upperSize.Y + lowerSize.Y + handSize.Y,
            upperSize.Z
        )
        
        -- Position left arm at the center of all left arm parts
        local upperPos = leftUpperArm.Position
        local lowerPos = leftLowerArm.Position
        local handPos = leftHand.Position
        leftArm.Position = (upperPos + lowerPos + handPos) / 3
    end
    ...
    if rightArm and rightUpperArm and rightLowerArm and rightHand then
        local upperSize = rightUpperArm.Size
        local lowerSize = rightLowerArm.Size
        ...
        local upperPos = rightUpperArm.Position
        local lowerPos = rightLowerArm.Position
        local handPos = rightHand.Position
        rightArm.Position = (upperPos + lowerPos + handPos) / 3
    end
    ...
    if leftLeg and leftUpperLeg and leftLowerLeg and leftFoot then
        local upperSize = leftUpperLeg.Size
        local lowerSize = leftLowerLeg.Size
        local footSize = leftFoot.Size
        leftLeg.Size = Vector3.new(
            upperSize.X,
            upperSize.Y + lowerSize.Y + footSize.Y,
            upperSize.Z
        )
        
        local upperPos = leftUpperLeg.Position
        local lowerPos = leftLowerLeg.Position
        local footPos = leftFoot.Position
        leftLeg.Position = (upperPos + lowerPos + footPos) / 3
    end
    ...
    if rightLeg and rightUpperLeg and rightLowerLeg and rightFoot then
        local upperSize = rightUpperLeg.Size
        local lowerSize = rightLowerLeg.Size
        local footSize = rightFoot.Size
        rightLeg.Size = Vector3.new(
            upperSize.X,
            upperSize.Y + lowerSize.Y + footSize.Y,
            upperSize.Z
        )
        
        local upperPos = rightUpperLeg.Position
        local lowerPos = rightLowerLeg.Position
        local footPos = rightFoot.Position
        rightLeg.Position = (upperPos + lowerPos + footPos) / 3
    end
    
    print("R15 to R6 conversion complete")
    return character
end
...
local function setGhostEnabled(newState)
    ...
    if ghostEnabled then
        ...
        -- Convert the ghost clone to R6-like structure for animations
        ghostClone = convertToR6Structure(ghostClone)
...
-- Modified to handle R6 animations on R15 character
local function playFakeAnimation(animationId)
    ...
    -- Get the joints from ghostClone (using R6 joint names for compatibility)
    ...
    -- Set up R6 joints mapping
    Joints["Torso"] = character:FindFirstChild("Torso") and character.Torso:FindFirstChild("Root")
    Joints["Head"] = character:FindFirstChild("Head") and character.Head:FindFirstChild("Neck")
    Joints["Left Arm"] = character:FindFirstChild("Left Arm") and character["Left Arm"]:FindFirstChild("Shoulder")
    Joints["Right Arm"] = character:FindFirstChild("Right Arm") and character["Right Arm"]:FindFirstChild("Shoulder")
    Joints["Left Leg"] = character:FindFirstChild("Left Leg") and character["Left Leg"]:FindFirstChild("Hip")
    Joints["Right Leg"] = character:FindFirstChild("Right Leg") and character["Right Leg"]:FindFirstChild("Hip")
    
    -- If R6 joints don't exist, try to map R15 joints to R6 equivalents
    if not Joints["Torso"] then
        Joints["Torso"] = rootPart:FindFirstChild("RootJoint")
    end
    
    if not Joints["Head"] then
        Joints["Head"] = character:FindFirstChild("Head") and character.Head:FindFirstChild("Neck")
    end
    
    if not Joints["Left Arm"] then
        Joints["Left Arm"] = character:FindFirstChild("LeftUpperArm") and character.LeftUpperArm:FindFirstChild("LeftShoulder")
    end
    
    if not Joints["Right Arm"] then
        Joints["Right Arm"] = character:FindFirstChild("RightUpperArm") and character.RightUpperArm:FindFirstChild("RightShoulder")
    end
    
    if not Joints["Left Leg"] then
        Joints["Left Leg"] = character:FindFirstChild("LeftUpperLeg") and character.LeftUpperLeg:FindFirstChild("LeftHip")
    end
    
    if not Joints["Right Leg"] then
        Joints["Right Leg"] = character:FindFirstChild("RightUpperLeg") and character.RightUpperLeg:FindFirstChild("RightHip")
    end
        ...
        pcall(function() -- Add pcall to handle errors gracefully
            ...
            while tick() - startTime < frameLength and not fakeAnimStop do
                ...
                for _, currentPose in pairs(currentFrame:GetDescendants()) do
                    ...
                    if motor and nextPose then
                        -- For R6 animations, we need to apply them to our R6-like structure
                        local originalC0 = ghostOriginalMotorCFrames[motor] and ghostOriginalMotorCFrames[motor].C0 or motor.C0
                        local currentCF = originalC0 * currentPose.CFrame
                        local nextCF = originalC0 * nextPose.CFrame
...
local function createAnimationListGui(animTextBox)
    ...
    titleLabel.Text = "Poison Hub R6 Animations"
    ...
    local function updateAnimationButtonsVisibility(searchText)
        ...
        for animName in pairs(BuiltInAnimationsR6) do
    ...
    local function refreshAnimationList()
        ...
        for animName, animId in pairs(BuiltInAnimationsR6) do
            ...
            if customAnimations[animName] then
                ...
                deleteAnimButton.MouseButton1Click:Connect(function()
                    ...
                    BuiltInAnimationsR6[animName] = nil
                    -- Save changes
    ...
    -- Create buttons for each animation in the BuiltInAnimationsR6 table
    for animName, animId in pairs(BuiltInAnimationsR6) do
        -- Create a container for each animation entry
        local animContainer = Instance.new("Frame")
        animContainer.Name = animName .. "Container"
...
local function createDraggableGui(getGhostEnabled, toggleGhost, getSizeValue, setSizeValue, getWidthValue, setWidthValue)
    ...
    creditLabel.Text = "R6 Reanimation by AK ADMIN"
    ...
    toggleButton.Text = "Enable R6 Reanimation"
    ...
    toggleButton.MouseButton1Click:Connect(function()
        ...
        if newState then
            toggleButton.Text = "Disable R6 Reanimation"
    ...
    -- Add button shadows
    local playButtonShadow.Parent = buttonContainer
    ...
    animListButton.Text = "Open R6 Animation List"
    ...
    animListButton.MouseButton1Click:Connect(function()
        if animationListGui then
            ...
            animListButton.Text = "Close R6 Animation List"
        end
    end)
    ...
    if input.UserInputType == Enum.UserInputType.Keyboard then
        for animName, keyCode in pairs(animationKeybinds) do
            if input.KeyCode == keyCode then
                ...
                if currentPlayingAnimation == BuiltInAnimationsR6[animName] then
                    -- If it's playing, stop it
                    stopFakeAnimation()
                else
                    -- If it's not playing or a different animation is playing, play this one
                    playFakeAnimation(BuiltInAnimationsR6[animName])
                end
